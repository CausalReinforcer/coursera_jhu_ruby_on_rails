...
class Assignment
  ...
  def create_user(params)
    # accept a hash of user properties (`:username` and `:password_digest`) as
    # an input parameter. Note these are 100% same as model class.
    # use the User Model class to create a new user in the DB
    # return an instance of the class with primary key (`id`),
    # and dates (`created_at` and `updated_at`) assigned
    user = User.new
  ...
  def create_todolist(params)
    # accept a hash of todolist properties (`:name` and `:due_date`)
    # as an input parameter. Note these are not 100% the same as Model class.
  ...
  def find_allusers(offset, limit)
    ...
    # `updated_at` ascending, with specified row offset and row limit.
  ...
  def find_alllists(offset, limit)
    # accept offset and limit input parameters
    # use the TodoList Model class to find all TodoLists, ordered by `list_due_date`
    # descending, with specified row offset and row limit.
  ...
  def find_user_byname(username)
    ...
    # NOTE:  Username is not unique in the Users table,
    # thus you can have many users with the same username.
  ...
  def find_todolist_byname(name)
    ...
    # NOTE: List name is not required to be unique,
    # thus you can have many lists with the same list name.
  ...
  def update_password(id, password_digest)
    ...
    # use the User Model class to update the `password_digest` for
    # the User associated with the id primary key.
  ...
  def update_listname(id, name)
    ...
    # use the TodoList Model class to update the `list_name` for
    # the TodoList associated with id primary key.
    # (no return is required)
  ...
  def delete_user(id)
    # accept an id input parameter
    # use the User Model class to remove the User associated with
    # the `id` primary key from the database.
    # (no return is required)
    User.find(id).delete
  end
  ...
  def delete_todolist(id)
    # accept an id input parameter
    # use the TodoList Model class to remove
    # the TodoList associated with the `id` primary key.
    # (no return is required)
